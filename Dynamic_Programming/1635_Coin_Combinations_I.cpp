...
void solve(){
    ll n,x;cin>>n>>x;
    vector<ll> c(n); in(c,n);
    ll ans = 0;
 
    vector<ll> dp(x+1,0);
    //dp[i] means i ko banane ke kitne tareeke hai using all coins
    dp[0] = 1;// don't take any means 1 way
 
    for(int i=1; i<=x; i++){
        //make money i using all coins
        for(int j=0; j<n; j++){
            if(i>=c[j]){
                //c[j],i-c[j]
                dp[i] += dp[i-c[j]];
