...
ll dp[19][2][10][2];
ll fun(string& s,int idx,int tight,int last,int lead){
    //lead means leading 0 continue or not
    //base case
    int n = s.length();
    if(idx==n) return 1;
 
    if(dp[idx][tight][last][lead] != -1) return dp[idx][tight][last][lead];
    ll ans = 0;
    if(tight==1){
        //0 to s[idx]-'0'
        ll mx = s[idx]-'0';
        //0 to mx-1 ke aage free
 
        for(int i=0; i<mx; i++){
            if(i==0 && lead){
                ans += fun(s,idx+1,!tight,i,lead);
            }else{
                if(i != last)
                ans += fun(s,idx+1,!tight,i,false);
            }
        }
 
        //mx ke aage strict
        if(lead && mx==0){
            ans += fun(s,idx+1,tight,mx,true);
        }
        if(mx != last){
            ans += fun(s,idx+1,tight,mx,false);
        }
    }else{
        //free
        //try all 0 to 9
    ...
    return dp[idx][tight][last][lead]=ans;
}
