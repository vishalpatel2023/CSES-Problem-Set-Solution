...
void solve(){
    ...
    //bfs
    vector<pair<char,pair<int,int>>> q,tmp;//char,x,y
    q.push_back({arr[0][0],{0,0}});
    string ans;
    ans += arr[0][0];
    vis[0][0] = 1;
    ...
    while(!q.empty()){
        // debug(q.size());
        //find min char reach from all available from q
        char mn = 'z';
        for(int i=0; i<q.size(); i++){
            int x = q[i].second.first;
            int y = q[i].second.second;
            //x,y-->x+1,y or x,y+1
            if(x+1<n && y<n) mn = min(mn,arr[x+1][y]);
            if(x<n && y+1<n) mn = min(mn,arr[x][y+1]);
        }
        if(mn=='z') break;
        tmp.clear();
        ...
        for(int i=0; i<q.size(); i++){
            ...
            int y = q[i].second.second;
            //x,y-->x+1,y or x,y+1
            if(x+1<n && y<n && (!vis[x+1][y]) && arr[x+1][y]==mn){
                tmp.push_back({mn,{x+1,y}});
                vis[x+1][y] = 1;
            }
            if(x<n && y+1<n && (!vis[x][y+1]) && arr[x][y+1]==mn){
                tmp.push_back({mn,{x,y+1}});
                vis[x][y+1] = 1;
            }
        }
        q = tmp;
