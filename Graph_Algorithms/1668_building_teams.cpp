...
bool fill(vector<int>& group,int node,vector<vector<int>>& adj){
    group[node] = 1;
    ...
    while(!q.empty()){
        ...
        for(auto x: adj[ptr.first]){
            if(group[x]==-1){
                group[x] = (!ptr.second);
                q.push({x,group[x]});
            }else if(group[x] != (!ptr.second)){
                return false;
            }
        }
    }
...
void solve(){
    ...
    for(int i=1; i<=n; i++){
        if(group[i]==0) group[i] = 2;
    }
    rep(i,1,n+1) cout<<group[i]<<" ";
    cout<<endl;
}
