...
void solve(){
    ...
    //let's find topo order
    queue<ll> q;
    vector<ll> topo;
    //push all node that has 0 indegree
    ...
    // print(topo,n);
    vector<ll> dp(n+1,-inf),parent(n+1,-1);
    dp[1] = 1;
    for(int i=0; i<n; i++){
        int curr = topo[i];
        //perform relaxation on edges
        for(auto node: adj[curr]){
            //dp[curr] + cost vs dp[node]
            if(dp[curr]+1>=dp[node]){
                dp[node] = dp[curr]+1;
                parent[node] = curr;
            }
        }
