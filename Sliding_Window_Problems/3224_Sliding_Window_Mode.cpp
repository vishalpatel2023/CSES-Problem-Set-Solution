...
void solve(){
    ...
    map<ll,ll> mp; //val freq
    set<pair<ll,ll>> st;//freq, val
    ...
    for(int i=k-1; i<n; i++){
        //include i
        auto ptr = mp.find(x[i]);
        if(ptr==mp.end()){
            mp[x[i]]++;
            st.insert({1,-x[i]});
        }else{
            auto tmp = st.find({ptr->second,-x[i]});
        ...
        //exclude i-(k-1) means reduce it's frequency
        ptr = mp.find(x[i-(k-1)]);
        auto pp = st.find({ptr->second,-x[i-(k-1)]});
        //both pointer will be valid
