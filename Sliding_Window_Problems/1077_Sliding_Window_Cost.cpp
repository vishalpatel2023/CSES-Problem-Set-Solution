...
void solve(){
    ...
    multiset<ll> mx,mn; //contains top half, botton half

    //insert first k values appropriately
    vector<ll> ans;

    ll sum1 = 0, sum2 = 0; //sum of mx multiset and sum of mn multiset elements

    for(int i=0; i<k; i++){
        int sz1 = mx.size(), sz2 = mn.size();
        if(sz1<sz2){
            mn.insert(x[i]); sum2 += x[i];
            mx.insert(*mn.rbegin()); sum1 += *mn.rbegin();
            sum2 -= *mn.rbegin(); mn.erase((--mn.end()));

        }else{
            if((int)mn.size()==0 || x[i]<*mn.rbegin()){
                mn.insert(x[i]);
                sum2 += x[i];
            }else{
                mx.insert(x[i]); sum1 += x[i];
                mn.insert(*mx.begin()); sum2 += *mx.begin();
                sum1 -= *mx.begin(); mx.erase(mx.begin()); 
            }
        }
    }
    ll med = *mn.rbegin();
    // debug(sum1) debug(sum2)
    ll res = sum1-((ll)mx.size())*med+med*(ll)(mn.size())-sum2;
    ans.push_back(res);

    for(int i=k; i<n; i++){
        //erase i-k and insert curr
        ll val = x[i-k];
        auto ptr = mn.find(val);
        if(ptr != mn.end()){
            sum2 -= *ptr; mn.erase(ptr); 
            if(mn.size()<mx.size()){
                mn.insert(*mx.begin()); sum2 += *mx.begin();
                sum1 -= *mx.begin(); mx.erase(mx.begin()); 
            }
        }else{
            sum1 -= val; mx.erase(mx.find(val));
        }
        //insert logic 
        int sz1 = mx.size(), sz2 = mn.size();
        if(sz1<sz2){
            sum2 += x[i]; mn.insert(x[i]); 
            mx.insert(*mn.rbegin()); sum1 += *mn.rbegin();
            sum2 -= *(--mn.end()); mn.erase((--mn.end())); 
