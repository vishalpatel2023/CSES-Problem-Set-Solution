...
void solve(){
    ...
    sort(vp.begin(),vp.end(),[](pair<ll,pair<ll,ll>>& p1,pair<ll,pair<ll,ll>>& p2){
    ...
    //dp2 means kya es range ko kisi ne rakha hai??
    ll mx = -1;
    for(int i=0; i<n; i++){
        if(vp[i].second.second<=mx){
            dp2[vp[i].first] = 1;
        }else{
            dp2[vp[i].first] = 0;
            mx = max(mx,vp[i].second.second);
        }
    }
    
    ll mini = 1e9+10;
    for(int i=n-1; i>=0; i--){
        if(vp[i].second.second>=mini){
            dp1[vp[i].first] = 1;
        }else{
            dp1[vp[i].first] = 0;
            mini = min(mini,vp[i].second.second);
        }
    }
    print(dp1,n);cout<<endl;
    print(dp2,n);cout<<endl;
}
