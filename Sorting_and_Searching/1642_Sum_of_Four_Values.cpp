...
bool fun(pair<ll,ll>& p1,pair<ll,ll>& p2){
    set<ll> st;
    st.insert(p1.first);st.insert(p2.first);
    st.insert(p1.second);st.insert(p2.second);
...
void solve(){
    ...
    for(auto& y: mp){
        ll req = x-y.first;
        auto ptr = mp.find(req);
        if(ptr==mp.end()) continue;
        if(y.first==req){
            auto& v = (ptr->second);
            int sz = (ptr->second).size();
            for(int i=0; i<sz; i++){
            ...
            for(int i=0; i<sz2; i++){
                for(int j=0; j<sz; j++){
                    if(fun(v2[i],v[j])){
                        cout<<v2[i].first<<" "<<v2[i].second<<" "<<v[j].first<<" "<<v[j].second<<endl;return;
                    }
                }
            }
        }
